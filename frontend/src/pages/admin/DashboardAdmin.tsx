...
export default function DashboardAdmin() {
  ...
  // Backend returns fecha_creacion (mapped to createdAt) and total as Decimal strings
  const ordenesEnRango = ordenes.filter((o: any) => {
    const fecha = new Date(o.createdAt || o.fecha_creacion);
    return !isNaN(fecha.getTime()) && fecha >= fechaDesde;
  });
  const getEstado = (o: any) => o.estado || o.estado_actual;
  const getTotal = (o: any) => {
    const t = o.total;
    return typeof t === 'string' ? parseFloat(t) : (typeof t === 'number' ? t : 0);
  };
  const ordenesCompletadas = ordenesEnRango.filter((o: any) => ['pagada', 'en_proceso', 'enviada', 'entregada'].includes(getEstado(o)));
  const ordenesPendientes = ordenesEnRango.filter((o: any) => getEstado(o) === 'pendiente_pago');
  const ventasTotales = ordenesCompletadas.reduce((sum: number, o: any) => sum + getTotal(o), 0);
          ...
          <CardContent className="p-0">
            <div className="divide-y max-h-[350px] overflow-y-auto">
              {ordenes.slice(0, 8).map((orden: any) => {
                ...
                const numero = orden.numero || orden.numero_orden || 'N/A';
                const fecha = orden.createdAt || orden.fecha_creacion;
                const total = getTotal(orden);
                const isCompleted = ['pagada', 'en_proceso', 'enviada', 'entregada'].includes(estado);
                const isPending = estado === 'pendiente_pago';
                return (
                <div key={orden.id} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-sm">{numero}</p>
                      <p className="text-xs text-muted-foreground">{orden.usuario?.nombre || 'N/A'} • {formatDate(fecha)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold">{formatCurrency(total)}</p>
                    <Badge variant={isCompleted ? 'default' : isPending ? 'secondary' : 'destructive'} className="text-xs">
                      {estado}
                    </Badge>
                  </div>
                </div>
            ...
            <div className="divide-y max-h-[300px] overflow-y-auto">
              {productos.slice(0, 6).map((producto: any) => (
                <div key={producto.id} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                  ...
                  <p className="font-bold">{formatCurrency(typeof producto.precio_venta === 'string' ? parseFloat(producto.precio_venta) : (producto.precio_venta || 0))}</p>
                </div>
              ))}
              {productos.length === 0 && (
